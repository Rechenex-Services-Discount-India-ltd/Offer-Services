document.addEventListener('DOMContentLoaded', () => {
    ...
    // Razorpay Configuration (Dynamic)
    let RAZORPAY_KEY_ID = 'rzp_live_Skk21ExrcBTgDs';
    
    // Listen for Key changes from Admin Panel
    database.ref('config/razorpay_key').on('value', (snapshot) => {
        if (snapshot.exists()) {
            RAZORPAY_KEY_ID = snapshot.val();
            console.log("Website RAZORPAY_KEY_ID updated:", RAZORPAY_KEY_ID);
        }
    });
    ...
    function selectSIM(simType) {
        ...
        const customerEmail = document.getElementById('customerEmail') ? document.getElementById('customerEmail').value.trim() : 'customer@rechargepe.com';
    ...
    function processPayment(amount, description, customerName, customerMobile, customerEmail) {
        const options = {
            ...
            prefill: {
                name: customerName || '',
                email: customerEmail || 'customer@rechargepe.com',
